...
namespace UnityTestCaseFramework
{
    ...
    {
        /// <summary>
        /// 字节数组最大长度，十六进制为单位，总容量为32MB。实际上也能超过，但是会警告
        /// </summary>
        static readonly int ENCODE_MAX_SIZE = 0x2000000;
        ...
        public void WriteFloat(float value)
        {
            ...
            Debug.AssertFormat(temp.Length == sizeof(float), "{0} != {1}", temp.Length, sizeof(float));
        ...
        public void WriteString(string str)
        {
            ...
            else
            {
                ...
                Debug.AssertFormat(strBytes.Length < ushort.MaxValue, "字符串过长：{0}:{1}", strBytes.Length, str);
        ...
        {
            BytesBuffer clone = new BytesBuffer(_size);
            clone.WriteBytes(_buffer, 0, _size);
            return clone;
        }
        ...
        internal void Expand(int sz)
        {
            if (_capacity - _size < sz)
            {
                ...
                {
                    Log.Error("byte buff is too large (>" + ENCODE_MAX_SIZE + ")");
                }
